...
import { faceCheckIn } from "../services/api";

const CAPTURE_INTERVAL_MS = 2000;

function stopCameraStream(webcamRef) {
    try {
...
export default function FaceCheckIn({ onResult }) {
    ...
    const captureFrame = useCallback(async () => {
        ...
        try {
            const res = await faceCheckIn(imageSrc);
                    ...
                    <div style={{ padding: "20px" }}>
                        <div style={{ marginBottom: 16 }}>
                            ...
                            <div style={{
                                ...
                                {result.employeeCode} · {result.position}
                            </div>
                        </div>

                        <div style={{
                            display: "grid",
                            gridTemplateColumns: result.checkOutTime ? "1fr 1fr" : "1fr",
                            gap: 12,
                        }}>
                            <TimeBlock label="Vào" time={formatTime(result.checkInTime)} />
                            {result.checkOutTime && (
                                <TimeBlock label="Ra" time={formatTime(result.checkOutTime)} />
                            )}
                        </div>
